                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <a href="{{route('hr.index',$idCompany=$department->company_id)}}">{{$department->company_name}}</a>
                            </div>
                        </div>
                        <hr>
                        ...
                        @forelse($workers as $item)
                            ...
                            <a href="{{route('hr.worker',$idWorker=$item->id)}}">{{ $item->secondName }} {{ $item->firstName }} {{ $item->middleName }}</a>
                        ...
                        <p class="text-center">
                            &nbsp;
                            <a href="{{route('hr.renamedepartment',$idDepartment=$department->department_id)}}" class="btn btn-primary pull-right btn-sm">
                                Редактировать
                            </a>
                            &nbsp;
                            @if ($department->is_delete)
                                <a href="{{route('hr.restoredepartment',['idDepartment'=>$department->department_id])}}" class="btn btn-danger pull-right btn-sm" onClick='return confirm("Действительно желаете восстановить данное подразделение?")'>
                                    Восстановить
                                </a>
                            @else
                                <a href="{{route('hr.deletedepartment',['idDepartment'=>$department->department_id])}}" class="btn btn-danger pull-right btn-sm" onClick='return confirm("Действительно желаете удалить данное подразделение?")'>
                                    Удалить
                                </a>
                            @endif
